...
public class PojavLauncher {
    ...
    public static void launchMinecraft(String[] args) throws Throwable {
        ...
        // 只能证明内存态；这里从磁盘重读 MC 即将加载的 options.txt，日志即可
        // 一锤定音"写入是否真的落到 MC 读的文件"。已知异常现场：26.3 运行期
        // 以 AFK 模式跑（SHORT_AFK=30fps 实锤），而启动器明明写了 minimized，
        // 分叉点就在这层（options.txt 残留重复行 → MC last-wins 读到 afk）。
        if ("1".equals(System.getenv("POJAV_DISABLE_VSYNC"))) {
