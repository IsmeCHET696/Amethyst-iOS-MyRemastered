...
//
// 根因链（26.3 client.jar 反编译实证）：
//   * MC 26.3 的 FramerateLimitTracker.getThrottleReason()：当
//     inactivityFpsLimit == AFK（InactivityFpsLimit 枚举仅 minimized/afk
//     两值）且 Util.getMillis() - latestInputTime > 60000ms 时返回
//     SHORT_AFK → getFramerateLimit() = min(maxFps, 30) = 30；
//   * Minecraft.runTick 尾部 framerateLimit < 260 时调
//     FramerateLimiter.limitDisplayFPS(framerateLimit)（LockSupport.parkNanos
//     睡到 33ms/帧）——渲染线程被 park 在该帧；
//   * iOS 上除触摸外没有持续的输入流，整合包加载 1-3 分钟无操作 →
//     latestInputTime 过期 → 整个加载期 + 挂机期被死死压在 30fps。
//     输入重置点仅在 MouseHandler.onButton/onScroll/onDrop/
//     handleAccumulatedMovement（后者还要求 isWindowActive）。
//
// 修复策略（双保险，与 Java 侧 options.txt 去重写入配合）：
//   1. MCOptionUtils.set() 去重（Task104）：清掉 options.txt 里残留的重复
//      行，避免 MC 侧 last-wins 读到历史遗留的 inactivityFpsLimit:afk；
//   2. 本心跳：窗口就绪后每 45s 推一个 (0,0) SDL_MOUSEWHEEL ——
