...
void glTexImage2D(GLenum target, GLint level, GLint internalFormat, GLsizei width, GLsizei height, GLint border,
    ...
    // things: GL_TEXTURE_INTERNAL_FORMAT is not an ES 3.0 pname (ANGLE rejects
    // it, Mesa tolerates it), the query reads whatever the CURRENT unit holds,
    // and a rejected allocation leaves an empty level. Record every depth
    // allocation with all three separated: the texture this layer believes is
    // bound, what the DRIVER says is bound on the active unit, whether the
    // upload raised an error, and the depth-bit count of what the driver
    // actually has on that binding.
    static int mg_depth_alloc_logged = 0;
    if (mg_depth_alloc_logged < 12 &&
        ...
        mg_depth_alloc_logged++;
        GLint err = GLES.glGetError();
        GLint drv_bound = 0;
        GLES.glGetIntegerv(GL_TEXTURE_BINDING_2D, &drv_bound);
        GLint depth_bits = -1;
        if (drv_bound != 0) {
            GLES.glGetTexLevelParameteriv(GL_TEXTURE_2D, level, GL_TEXTURE_DEPTH_SIZE, &depth_bits);
            GLES.glGetError(); // clear the query error if the level is not allocated
        }
        const TextureObject* shadow_tex = mgGetTexObjectByTarget(target);
        LOG_W_FORCE("[MG] depth alloc #%d: %s %dx%d, shadow tex %u, driver-bound tex %d, upload err 0x%x, depth bits %d",
                    mg_depth_alloc_logged, glEnumToString(internalFormat), width, height,
                    shadow_tex ? shadow_tex->texture : 0, drv_bound, err, depth_bits)
        if (err != GL_NO_ERROR) {
            LOG_W_FORCE("[MG] note: depth allocation raised an error -- the level is empty and every FBO using it reads garbage")
        }
    }
